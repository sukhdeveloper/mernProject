...
const MyTeacherChat = () => {
  ...
  useEffect(() => {
    dispatch(getFrontUserDetailForTeacher()).then((res) => {
      if (res && res.success) {
        ...
        return (
          <button
            ...
            onClick={() =>
              selectChannel(
                channel.id,
                newArray[0]?.student_id?.first_name,
                newArray[0]?.student_id?.profile_image,
                true,
                newArray[0]?.student_id?._id
              )
            }
          >
            <div className="str-chat__channel-preview-messenger--left">
              ...
              >
                <img
                  ...
                  className="str-chat__avatar-image str-chat__avatar-image--loaded"
                  data-testid="avatar-img"
                  src={newArray[0]?.student_id?.profile_image}
                  style={{
                    flexBasis: "40px",
                    height: "40px",
                    objectFit: "cover",
            ...
            <div className="str-chat__channel-preview-messenger--right">
              <div className="str-chat__channel-preview-messenger--name">
                <span>{newArray[0]?.student_id?.first_name}</span>
              </div>
              <div className="str-chat__channel-preview-messenger--last-message">
                <p>{latestMessage}</p>
              </div>
  ...
  const blockUnblockUserForTeacher = (blockingStatus) => {
    var blockData = {};
    blockData.student_id = studentId;
    blockData.frozen_channel = blockingStatus;
    dispatch(blockUnblockChannelForTeacher(blockData)).then((res) => {
      if (res && res.success) {
                ...
                >
                  {showBlockButton && channelBlockingDetails?.showInputField && (
                    <Button
                      variant="contained"
                      onClick={() =>
                        blockUnblockUserForTeacher(
                          !channelBlockingDetails?.blockStatusForTeacher
                        )
                      }
                    >
                      {!channelBlockingDetails?.blockStatusForTeacher
