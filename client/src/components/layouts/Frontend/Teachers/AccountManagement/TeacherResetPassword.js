...
const TeacherResetPassword = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
          dispatch(reset_password(values)).then((res)=>{  
              if (res && res.success) {
                    history.push({
                      pathname: "/validation-teacher-reset-pass",
    ...
    return (
        <Box className="SignUpFrom ResetPass">
              <Grid container>
                   <Grid item md={5} lg={4} xs={12} className="m-auto">
                        <Box className="FormOuter">
                             ...
                             {formik.errors.phone && formik.touched.phone?(<div className='error_message'>{formik.errors.phone}</div>) : null}
                                ...
                                <Link to="/teacher/sign-in" className="BackToHome mt-4"><BiLeftArrowCircle />Back to login</Link>
                        </Box>
                   </Grid>
                </Grid>
