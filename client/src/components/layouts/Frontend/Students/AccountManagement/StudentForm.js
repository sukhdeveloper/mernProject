  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const formdata = {
        ...values,
        accept_terms_and_conditions:
          (values.accept_terms_and_conditions.length = 1),
        fcmToken: fcm_token,
        ipAddress: Ip,
        deviceId: null,
        platform: "web",
  ...
  return (
    <Box>
      <FormControl variant="standard" className="FiledsOuter InputsWrap">
        <div className="input-group students_phn">
          <span className="input-group-text" id="student_phonee">
            <IoIosPhonePortrait />
          </span>
          <span
          ...
          <input
            ...
            aria-describedby="student_phonee"
            name="phone"
      ...
      <p>
        ...
        <Link to="/login" className="acc-link">
          Log in
        </Link>
      </p>
      <br/>
