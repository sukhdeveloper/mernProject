...
function PaymentData({
  ...
  class_id,
  transaction: {
    loadingForAllTransactions,
    transactionsPerPage,
    totalPagesForPayment,
  },
}) {
  var transactionData = [];
  ...
  for (let i = 0; i < transactionsPerPage.length; i++) {
    ...
    transactionData.push({
      ...
      fullName:
        transactionsPerPage[i].user_id.first_name + " " + transactionsPerPage[i].user_id.last_name,
      profile_image: transactionsPerPage[i].user_id.profile_image,
      user_id: transactionsPerPage[i].user_id._id,
              ...
              <ListItem alignItems="center">
                <ListItemText>
                  <Typography variant="h5">
                    <Link to={`/user-details/${row.user_id}`}>{value}</Link>
                  </Typography>
                </ListItemText>
              </ListItem>
            </List>
